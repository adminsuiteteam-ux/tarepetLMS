...
export default function AdminCBTApproval() {
  ...
  const fetchExams = () => {
    authClient.get('/assessments/cbt-exams/').then(res => {
  ...
  const handleApprove = async (examId: number) => {
    try {
      await authClient.post(`/assessments/cbt-exams/${examId}/approve/`);
      fetchExams();
      setView('queue');
  ...
  const handleReject = async (examId: number) => {
    try {
      await authClient.post(`/assessments/cbt-exams/${examId}/reject/`, { reason: rejectReason });
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-violet-50 p-4 md:p-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-slate-900">CBT Approval Hub</h1>
              <p className="text-slate-500 text-sm">{pendingExams.length} exam(s) pending your approval</p>
            </div>
          </div>
          ...
          {pendingExams.length > 0 && (
            <div className="mb-8">
              <h2 className="text-lg font-bold text-amber-700 mb-3 flex items-center gap-2"><AlertTriangle className="w-5 h-5" /> Pending Approval</h2>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-400 mt-1">Submitted by: {exam.teacher_name}</p>
                      </div>
          ...
          {pendingExams.length === 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center mb-8">
              ...
              <p className="text-slate-400">No pending exams requiring your approval.</p>
            </div>
          )}
          ...
          {otherExams.length > 0 && (
            <div>
              <h2 className="text-lg font-bold text-slate-700 mb-3">All Exams</h2>
                  ...
                  <div key={exam.id} className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${STATUS_STYLES[exam.status] || ''}`}>{exam.status}</span>
                        <span className="text-xs text-slate-400">{exam.assessment_type === 'TEST' ? 'C.A. Test' : 'Exam'}</span>
                      </div>
          ...
          {showRejectModal && selectedExam && (
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowRejectModal(false)}>
              <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                <h3 className="text-lg font-bold text-slate-900 mb-2">Reject: {selectedExam.title}</h3>
                <p className="text-sm text-slate-500 mb-4">Provide feedback for the teacher (optional):</p>
                <textarea
                ...
                <div className="flex gap-3">
                  <button onClick={() => setShowRejectModal(false)} className="flex-1 h-11 rounded-xl border border-slate-200 text-slate-600 font-medium hover:bg-slate-50 transition">Cancel</button>
                  <button onClick={() => handleReject(selectedExam.id)} className="flex-1 h-11 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 transition">Reject Exam</button>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-slate-900">Previewing: {selectedExam.title}</h1>
            </div>
            <div className="flex gap-2">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-slate-400">Course:</span> <strong>{selectedExam.course_detail?.name}</strong></div>
              <div><span className="text-slate-400">Type:</span> <strong>{selectedExam.assessment_type === 'TEST' ? 'C.A. Test' : 'Final Exam'}</strong></div>
              <div><span className="text-slate-400">Term:</span> <strong>{selectedExam.term.replace('_', ' ')}</strong></div>
              <div><span className="text-slate-400">Duration:</span> <strong>{selectedExam.duration_minutes} minutes</strong></div>
              <div><span className="text-slate-400">Questions/Page:</span> <strong>{selectedExam.questions_per_page}</strong></div>
              <div><span className="text-slate-400">Total Questions:</span> <strong>{selectedExam.questions_count}</strong></div>
            </div>
          ...
          <h3 className="font-bold text-slate-700 mb-3">Questions Preview</h3>
          <div className="space-y-3">
          ...
          {showRejectModal && (
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowRejectModal(false)}>
              <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                ...
                />
                <div className="flex gap-3">
                  <button onClick={() => setShowRejectModal(false)} className="flex-1 h-11 rounded-xl border border-slate-200 text-slate-600 font-medium hover:bg-slate-50">Cancel</button>
                  <button onClick={() => handleReject(selectedExam.id)} className="flex-1 h-11 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700">Reject</button>
                </div>
              </div>
            </div>
          )}
