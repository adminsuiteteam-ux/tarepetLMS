...
export default function TeacherDashboard() {
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('tarepet_submission_received', handleSubmissionReceived);
      window.removeEventListener('cbt_store_updated', handleSubmissionReceived);
    };
  }, []);
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {ex.is_locked ? (
                          <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-amber-100 text-amber-900 border border-amber-300 flex items-center gap-1">
                            <Lock className="w-3 h-3 text-amber-700" /> Locked by Examiner
                          </span>
                        ) : (
                          <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-blue-100 text-blue-900 flex items-center gap-1">
                            <Unlock className="w-3 h-3 text-blue-700" /> Submissions Open
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <button
                        ...
                        onClick={async (e) => {
                          e.stopPropagation();
                          const willLock = !ex.is_locked;
                          const confirmed = await showConfirm({
                            title: willLock ? 'Lock Exam for Students?' : 'Unlock Exam for Students?',
                            message: willLock
                          ...
                          if (confirmed) {
                            await toggleExamLocked(ex.id, willLock);
                            setTeacherExams(getStoredExams());
                            showToast(willLock ? `Exam "${ex.title}" locked.` : `Exam "${ex.title}" unlocked.`);
                          }
                        }}
                        className={`text-xs font-bold px-3 py-1.5 rounded-xl flex items-center gap-1 transition-colors cursor-pointer shadow-2xs ${
                          ex.is_locked
                      ...
                      >
                        {ex.is_locked ? <Lock className="w-3.5 h-3.5 text-amber-700" /> : <Unlock className="w-3.5 h-3.5" />}
                        {ex.is_locked ? 'Unlock Exam' : 'Lock Exam'}
                      </button>
                      <button
                        type="button"
                        onClick={(e) => {
      ...
      <>
        ...
        <div className="flex items-center justify-between gap-2 sm:gap-4 pb-3 border-b border-border w-full">
          <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 flex-1">
            <span className="w-3 h-3 rounded-full bg-emerald-500 shrink-0 shadow-xs ring-4 ring-emerald-500/20" />
            <h2 className="text-sm sm:text-xl font-bold font-serif text-foreground truncate">{t('teacher.profile_title', 'Teacher Official Profile')}</h2>
          </div>
          <div className="relative shrink-0 ml-auto">
            <button
              onClick={() => setShowActionsDropdown(prev => !prev)}
              className="px-3.5 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm cursor-pointer"
            >
          ...
          <div className="fixed inset-0 z-[100] bg-black/75 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-hidden">
            <div className="bg-card border border-border rounded-3xl max-w-3xl w-full shadow-2xl flex flex-col max-h-[90vh] overflow-hidden relative">
              ...
              >
                ...
                <div className="p-4 sm:p-6 overflow-y-auto space-y-6 flex-1 text-xs pb-10">
                    ...
                    <div className="space-y-2 flex-1 text-center sm:text-left">
                      <input
                        ...
                        onChange={e => {
                          ...
                          if (file) {
                            if (file.size > 10 * 1024 * 1024) {
                              showToast(t('teacher.image_size_limit', 'Image size exceeds 10MB limit.'));
                              return;
                            }
                            const reader = new FileReader();
                            reader.onloadend = () => {
                        ...
                        {profileForm.profileImage && (
                          <>
                            ...
                            >
                              ...
                              <span>{t('teacher.remove_photo', 'Remove')}</span>
                            </button>
                          </>
                        )}
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-muted-foreground uppercase block mb-1">{t('teacher.form_teacher_class_assignment', 'Form Teacher Class Assignment')}</label>
                        <select
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-muted-foreground uppercase block mb-1">{t('teacher.date_joined_faculty_label', 'Date Joined Faculty')}</label>
                        <input
                          type="text"
                ...
                {/* Sticky Modal Footer (Permanently Visible above Bottom Navbar) */}
