  ...
  try {
    ...
    if (saved) {
      ...
      if (Array.isArray(parsed)) {
        ...
        const existingCodes = new Set(liveOnly.map((s: any) => String(s.code || s.studentId || '').toLowerCase()));
        const missingDefaults = DEFAULT_STUDENTS.filter(d => !existingCodes.has(String(d.code).toLowerCase()) && !isAccountDeleted(d.code) && !isAccountDeleted(d.name));
