    ...
    <div className="w-full min-h-screen pb-16 relative overflow-hidden" style={{ fontFamily: 'var(--font-poppins)' }}>
      ...
      <div className="pt-2 pb-4 space-y-4">
        {onBack && (
          <button
            onClick={onBack}
            className="w-10 h-10 rounded-full bg-card/90 border border-border/80 shadow-xs flex items-center justify-center text-foreground hover:bg-accent transition-all active:scale-90"
            title="Go Back"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
        )}
        <h1 className="text-2xl font-bold font-serif text-foreground tracking-tight px-1">
      ...
      <div className="bg-card rounded-3xl border border-border/70 shadow-lg p-6 text-center space-y-3.5 relative overflow-hidden mb-5">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground font-normal break-all">
            {email || 'user@tarepet.com'}
          </p>
          <div className="flex items-center justify-center gap-1.5 text-[11px] text-muted-foreground font-medium pt-0.5 flex-wrap">
            <span>{subtitle || roleTitle}</span>
            {location && (
              <>
                <span>•</span>
