...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 5. Create 5 Official Student Accounts
        students_seed_roster = [
            {
                'email': 'emeka.amadi@tarepet.com',
                'first_name': 'Emeka',
                'last_name': 'Amadi',
                'student_id': 'TMS/SS1/SCI/4821',
                'grade_level': 'SS1',
            ...
            },
            {
                ...
                'first_name': 'Chidinma',
                'last_name': 'Okoro',
                'student_id': 'TMS/JSS2/0042',
                'grade_level': 'JSS2',
                'house': 'Yellow House (Cheetah)',
            },
            {
                'email': 'kelechi.eze@tarepet.com',
                'first_name': 'Kelechi',
                'last_name': 'Eze',
                'student_id': 'TMS/PRI4/0018',
                'grade_level': 'Primary 4',
                'house': 'Green House (Falcon)',
            },
            {
                'email': 'somto.nnamdi@tarepet.com',
                'first_name': 'Somtochukwu',
                'last_name': 'Nnamdi',
                'student_id': 'TMS/SS2/ART/0031',
                'grade_level': 'SS2',
                'house': 'Red House (Lion)',
            },
            {
                'email': 'tari.powei@tarepet.com',
                'first_name': 'Tari',
                'last_name': 'Tari-Powei',
                'student_id': 'TMS/PRI1/0009',
        ...
        for s_data in students_seed_roster:
            s_user = User.objects.filter(email=s_data['email']).first()
            if not s_user:
                s_user = User.objects.create_user(
                    email=s_data['email'],
                    password=s_data['student_id'],
                    first_name=s_data['first_name'],
                    last_name=s_data['last_name'],
                    role=User.Role.STUDENT,
                )
            else:
                s_user.first_name = s_data['first_name']
                s_user.last_name = s_data['last_name']
                s_user.set_password(s_data['student_id'])
                s_user.save()

            s_prof, _ = StudentProfile.objects.get_or_create(
                user=s_user,
                defaults={
                    'student_id': s_data['student_id'],
                    'grade_level': s_data['grade_level'],
                    'house': s_data['house'],
                }
            )
            s_prof.student_id = s_data['student_id']
            s_prof.grade_level = s_data['grade_level']
            s_prof.house = s_data['house']
            s_prof.save()
            s_prof.parents.add(parent_prof)

            if primary_student_prof is None:
                primary_student_prof = s_prof
